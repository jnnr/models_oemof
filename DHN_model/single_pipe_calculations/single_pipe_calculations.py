...
def plot_data():
    ...
    ylim = [(0, 3), (0, 1.5), (0, 30), (0.1, 0.5), (0, 30)]
    colors = [sns.color_palette("hls", len(sam_results[coord])).as_hex() for coord in coords]
    labels = [[f'{coord}={D}' for D in sam_results.coords[coord].values] for coord in coords]
    ...
    for i, c in enumerate(['v [m/s]',
        ...
        for ii, vv in enumerate(sam_results.coords['D'].values):
            sam_results.squeeze() \
            .isel(k=1, DT_prod_in=-1) \
            .sel(results=c, D=vv) \
            .plot(ax=axs[i,0], marker='.',
                  markersize=10, color=colors[0][ii])
        ...
        for ii, vv in enumerate(sam_results.coords['DT_prod_in'].values):
            sam_results.squeeze() \
                .isel(k=1, DT_prod_in=ii) \
                .sel(results=c, D=0.25) \
                .plot(ax=axs[i, 1], marker='.',
                      markersize=10, color=colors[1][ii])
        ...
        for ii, vv in enumerate(sam_results.coords['k'].values):
            sam_results.squeeze() \
                .isel(k=ii, DT_prod_in=-1) \
                .sel(results=c, D=0.25) \
                .plot(ax=axs[i, 2], marker='.',
                      markersize=10, color=colors[2][ii])
    ...
    for i, coord in enumerate(coords):
        axs[-1,i].legend(axs[-1,i], labels=labels[i], loc='lower center',
        bbox_to_anchor=(0.5, -1.8), ncol=1)
